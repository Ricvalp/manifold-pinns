...
def get_config():
    ...
    config.dataset.charts_path = env_path(
        "MANIFOLD_PINNS_EIKONAL_BUNNY_CHARTS_PATH",
        data_path("bunny", "charts_1"),
    )
    ...
    config.sparse_points.path = env_path(
        "MANIFOLD_PINNS_EIKONAL_BUNNY_SPARSE_POINTS_PATH",
        run_path("sparse_points", "eikonal", "bunny"),
    )
    ...
    config.autoencoder_checkpoint.checkpoint_path = env_path(
        "MANIFOLD_PINNS_UAE_BUNNY_CHECKPOINT",
        checkpoint_path("uae", "bunny", "latest"),
    )
    config.autoencoder_checkpoint.step = env_int("MANIFOLD_PINNS_UAE_BUNNY_STEP", 0)
    ...
    training.batches_path = batch_path("eikonal", "bunny")
    ...
    profiler.log_dir = profiler_path("eikonal", "bunny")

    # Saving
    config.saving = saving = ml_collections.ConfigDict()
    saving.checkpoint_dir = checkpoint_path("pinn", "eikonal", "bunny")
    ...
    eval.checkpoint_dir = env_path(
        "MANIFOLD_PINNS_EIKONAL_BUNNY_CHECKPOINT",
        saving.checkpoint_dir,
    )
    eval.step = 179999
    eval.N = 2000
    eval.use_existing_solution = False
    eval.solution_path = eval_path("eikonal", "bunny")
