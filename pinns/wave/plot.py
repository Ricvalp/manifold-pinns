...
def plot_solutions(charts, ts, values=None, figsize=(3, 3), dpi=300, 
                         cmap='magma', alpha=0.8, s=5, view_angle=(20, 45),
                         save_dir=None, zlim=None, vmin=None, vmax=None,
                         show_time=True, file_prefix="chart_t_", every_n=1,
                         show_colorbar=False, save_pdf=False):
    """
    ...
    for i, (chart, t) in enumerate(zip(selected_charts, selected_ts)):
        # Create a new figure for each chart with tight layout from the start
        fig = plt.figure(figsize=figsize, dpi=dpi, constrained_layout=True)
        
        # Use the entire figure space for the subplot
        ax = fig.add_subplot(111, projection="3d")
        ...
        if save_dir:
            ...
            # Use pad_inches=0 to remove padding and bbox_inches='tight' for tight cropping
            plt.savefig(filepath, bbox_inches='tight', dpi=dpi, pad_inches=0.1)
            
            # Also save as PDF with the same tight settings
            if save_pdf:
                pdf_path = str(filepath).rsplit('.', 1)[0] + '.pdf'
                plt.savefig(pdf_path, bbox_inches='tight', format='pdf', pad_inches=0.1)
        
        # Close the figure to free memory
        plt.close(fig)


def plot_2d_solutions(solutions, ts, figsize=(3, 3), dpi=300, 
                         cmap='magma',
                         save_dir=None, vmin=None, vmax=None,
                         show_time=True, file_prefix="chart_2d_t_", every_n=1,
                         save_pdf=False):
    """
    Plot each 2D chart individually and save to separate files.
    ...
    # Select charts at regular intervals
    selected_indices = range(0, len(solutions), every_n)
    selected_ts = ts[selected_indices]
    selected_solutions = [solutions[i] for i in selected_indices]
    
    
    # Process each selected chart individually
    for i, (sol, t) in enumerate(zip(selected_solutions, selected_ts)):
