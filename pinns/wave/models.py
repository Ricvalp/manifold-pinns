...
class WaveTime(MPINNSingleChart):
    def __init__(
    ...
    def create_losses(self):
        ...
        def compute_derivative_ics_loss(params, x, y, ics_derivative):
            u_t_net = grad(self.u_net, argnums=3)
            u_t_pred = vmap(u_t_net, (None, 0, 0, None))(params, x, y, 0.0)
            return jnp.mean((u_t_pred - ics_derivative) ** 2)
    ...
    def r_net(self, params, conditioning, x, y, t):
        u_t = grad(self.u_net, argnums=3)
        u_tt = grad(u_t, argnums=3)(params, x, y, t)
        return u_tt - self.c2 * self.laplacian_net(params, conditioning, x, y, t)

    def losses(self, params, batch):

        res_batches, ics_batches, ics_derivative_batches = batch
        ...
        x_derivative, y_derivative = ics_derivative_input_points[:, 0], ics_derivative_input_points[:, 1]

        ics_derivative_loss = self.compute_derivative_ics_loss(params, x_derivative, y_derivative, ics_derivative_values)
        ...
        loss_dict = {"ics": ics_loss, "ics_derivative": ics_derivative_loss, "res": res_loss}
